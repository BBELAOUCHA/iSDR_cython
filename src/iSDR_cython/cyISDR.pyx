...
cpdef cd_mneiSDR(np.ndarray[floating, ndim=1] w,
    ...
    """Cython version iSDR
        ...
        (1/2) * norm(y - X w, 2)^2 |p->T + alpha norm(w)_21 + (1/2) * norm(y - G w, 2)^2 |0->T
    """
    if floating is float:
        dtype = np.float32
    else:
    ...
    with nogil:
        ...
        for n_iter in range(max_iter):
            ...
            for f_iter in range(n_s):  # Loop over coordinates
                ...
                for i in range(m_p - 1):
                    # First and last block
                    tmp[i] += mu_X[ii]*(_dot(n_c, &G[0, 0] + ii*n_c, 1,
                              &R[0] + i * n_c, 1))
                    for j in range(i + 1):
                        tmp[i] += mu_X[ii]*(_dot(n_c, X_ptr + ii * n_c + (i - j) * block, 1,
                              &R[0] + j * n_c + m_p*n_c, 1))
                ...
                for i in range(m_p - 1, n_t_s - m_p + 1):
                    for j in range(m_p):
                        tmp[i] += mu_X[ii]*(_dot(n_c, X_ptr + ii*n_c + (m_p - 1 - j) * block, 1, &R[0] + (j + i + 1) * n_c , 1))

                tmp[m_p - 1] += mu_X[ii]*(_dot(n_c, &G[0, 0] + ii*n_c, 1, &R[0] + (m_p - 1) * n_c, 1))
                nn = _nrm2(n_t_s, &tmp[0], 1)
                # copy tmp back to w[ii, :]
                s = fmax(nn, mu_X[ii] * alpha)
                _scal(n_t_s, fmax(1.0 - (mu_X[ii] * alpha) / s, 0.0), &tmp[0], 1)
            ...
            if w_max == 0.0 or d_w_max / w_max < d_w_tol or n_iter == max_iter - 1:
                ...
                for ii in range(n_s):
                    for jj in range(m_p - 1, n_t_s - 1):
                        s = 0.0
                        for i in range(m_p):
                            s += (_dot(n_c, X_ptr + ii*n_c + (m_p - 1 - i) * block, 1, &R[0] + (j + jj + 1) * n_c , 1))
                        XtA[ii, jj] = s

                    XtA[ii, m_p - 1] += (_dot(n_c, &G[0, 0] + ii*n_c, 1, &R[0] + (m_p - 1) * n_c, 1))
